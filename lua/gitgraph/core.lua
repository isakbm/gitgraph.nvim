...
local function process_raw_commits(raw_commits)
  ...
  local dur = os.clock() - start
  log.info('process_raw_commits dur:', dur * 1000, 'ms')
...
local function populate_child_parent_data(commits, sorted_commits)
  ...
  local dur = os.clock() - start
  log.info('populate_child_parent_data dur:', dur * 1000, 'ms')
end

---@param sym I.GGSymbols
---@param opt I.DrawOptions
---@return I.GGSymbols
local function get_symbols(sym, opt)
  ---@type I.GGSymbols
  return vim.tbl_extend('force', sym, {
    GFORKU = opt.pretty and '⓵' or sym.GFORKU, -- '┼',
    GFORKD = opt.pretty and '⓴' or sym.GFORKD, -- '┼',
    GRUDCD = opt.pretty and '⓶' or sym.GRUDCD, -- '├',
    GRUDCU = opt.pretty and '⓸' or sym.GRUDCU, -- '├',
    GLUDCD = opt.pretty and '⓷' or sym.GLUDCD, -- '┤',
    GLUDCU = opt.pretty and '⓹' or sym.GLUDCU, -- '┤',
    GLRDCL = opt.pretty and 'ⓢ' or sym.GLRDCL, -- '┬',
    GLRDCR = opt.pretty and 'ⓣ' or sym.GLRDCR, -- '┬',
    GLRUCL = opt.pretty and 'ⓥ' or sym.GLRUCL, -- '┴',
    GLRUCR = opt.pretty and 'ⓤ' or sym.GLRUCR, -- '┴',

    commit = opt.pretty and 'ⓚ' or sym.commit,
    merge_commit = opt.pretty and '⓮' or sym.merge_commit,
    commit_end = opt.pretty and 'ⓛ' or sym.commit_end,
    merge_commit_end = opt.pretty and '⓯' or sym.merge_commit_end,
  })
end
...
local function insert_vert_and_hor_pipes(graph, sym)
  ...
  local dur = os.clock() - start
  log.info('insert_vert_and_hor_pipes dur:', dur * 1000, 'ms')
end
...
local function insert_symbols_on_connector_rows(graph, sym)
  ...
  local dur = os.clock() - start
  log.info('insert_symbols_on_connector_rows dur:', dur * 1000, 'ms')
end
...
---@return I.Highlight[]
---@return integer?
local function graph_to_lines(options, graph, sym, fields, commits)
  ...
  ---@return I.Highlight[]
  local function row_to_highlights(row, row_idx, continuation_symbols)
    local row_hls = {}
    local offset = 0
  ...
  for idx = 1, #graph do
    ...
    if options.mode ~= 'test' then
      ...
      for _, hl in ipairs(row_to_highlights(proper_row, idx, continuation_symbols)) do
  ...
  local dur = os.clock() - start
  log.info('graph_to_lines dur:', dur * 1000, 'ms')
...
local function straight_j(commits, sorted_commits)
  ...
  local dur = os.clock() - start
  log.info('straight_j dur:', dur * 1000, 'ms')

  return graph, found_bi_crossing
end
---
